defmodule Meme do
  ...
  defmacro defmemo(
             {:when, when_meta, [{name, meta, raw_args} | guards]},
             compiletime_params = [_ | _],
             do: body
           ) do
  ...
  defmacro defcached(
             raw_definition = {:when, when_meta, [{name, meta, raw_args} | guards]},
             compiletime_params = [_ | _],
             do: body
           ) do
    {arg_names, decorated_args} = decorate_args(raw_args)
  ...
  defmacro defcached(raw_definition = {name, meta, raw_args}, compiletime_params = [_ | _],
             do: body
           ) do
    {arg_names, decorated_args} = decorate_args(raw_args)
    cached_name = String.to_atom("cached_#{name}")
  ...
  defmacro defcachedp(raw_definition = {name, meta, raw_args}, compiletime_params = [_ | _],
             do: body
           ) do
    {arg_names, decorated_args} = decorate_args(raw_args)
    cached_name = String.to_atom("cached_#{name}")
  ...
  defp meme_under_the_hood(name, args, code, compiletime_params = [_ | _]) do
    ...
    conditional_caching_ast =
      ...
      |> case do
        nil ->
          quote do
            {:ok, true} =
              Cachex.put(:meme, key, {:exists, value},
                ttl: unquote(timeout),
                expire: unquote(timeout),
                local: true
              )
          end
    ...
    quote do
      ...
      case Cachex.get(:meme, key, local: true) do
        {:ok, nil} ->
          value = unquote(code)
